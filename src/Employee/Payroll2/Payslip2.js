...
import "../../components/Payroll/DisplaySalaryData.css";
...
const DisplayPaySlipData = () => {
    ...
    const fetchEmployeePayslips = async () => {
      try {
        ...
        const response = await axios.get(
          `${ BASE_URL }/get_employee_payslips?employeeId=${loggedInEmployee.employeeid}`
        );
  ...
  useEffect(() => {
    const filtered = salaryData.filter((item) => {
      ...
      const joiningDate = (item.joiningDate || "").toString();
      const workingDays = (item.workingDays || "").toString();
      const filterLowerCase = filterValue.toLowerCase();
      
      return (
        employeeId.toLowerCase().includes(filterLowerCase) ||
        month.toLowerCase().includes(filterLowerCase) ||
        year.toLowerCase().includes(filterLowerCase) ||
        joiningDate.toLowerCase().includes(filterLowerCase) ||
        workingDays.toLowerCase().includes(filterLowerCase)
    ...
    if (startDate && endDate) {
      const filteredByDate = filtered.filter((item) => {
        const payslipDate = new Date(item.month + " 1, " + item.year);
  ...
  }, [filterValue, salaryData, startDate, endDate]);
  ...
  return (
    <div className="mothercontainer-admin">
      ...
      <div className="bg">
        {/* <h4 className="heading409">PaySlip</h4> */}
  ...
  showMonthYearPicker
  dateFormat="MM/yyyy" // Display month and year only
/>
<DatePicker
  selected={endDate}
...
/> */}
          ...
          >
            ...
            <g id="SVGRepo_iconCarrier" >
              {" "}
              <rect fill="white"  className="size767"></rect>
              <path
              className="size767"
                fill=""
        ...
        <div className="table2314">
          <table className="table654">
            <thead className="headt76">
              <tr>
                <th>Employee ID</th>
                <th>EmployeeName</th>
                <th>Department</th>
            ...
            <tbody>
              {filteredData.map((salaryItem) => (
                <tr key={salaryItem._id}>
                  <td className="td12">{salaryItem.employeeId}</td>
                  <td className="td12">{salaryItem.employeeName}</td>
                  <td className="td12">{salaryItem.department}</td>
                  <td className="td12">{salaryItem.designation}</td>
                  <td className="td12">{salaryItem.month}</td>
                  <td className="td12">{salaryItem.year}</td>
                  <td className="td12">{salaryItem.joiningDate}</td>
                  <td className="td12">{salaryItem.workingDays}</td>
                  <td className="td12">{salaryItem.accountNumber}</td>
                  <td className="td12">{salaryItem.bankName}</td>
                  <td className="td12">{salaryItem.UANNumber}</td>
                  <td className="td12">{salaryItem.PanNumber}</td>
                  <td className="td12">{salaryItem.basicSalary}</td>
                  <td className="td12">{salaryItem.houseRentAllowance}</td>
                  <td className="td12">{salaryItem.travellingAllowance}</td>
                  <td className="td12">{salaryItem.medicalAllowance}</td>
                  <td className="td12">{salaryItem.dearnessAllowance}</td>
                  <td className="td12">{salaryItem.pf}</td>
                  <td className="td12">{salaryItem.professionalTax}</td>
                  <td className="td12">{salaryItem.grossSalary}</td>
                  <td className="td12">{salaryItem.netSalary}</td>
                  <td className="td12">
                    <button
                     className="change31"
                      onClick={() => downloadAsPDF(salaryItem)}
                    >
                     <FaCloudArrowDown /> Download
                    </button>
                  </td>
                </tr>
              ))}
...
export default DisplayPaySlipData;
